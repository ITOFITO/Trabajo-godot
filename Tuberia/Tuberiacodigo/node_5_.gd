...
func _ready():
	...
	if randi() % 100 < 40:  # randi() genera un número aleatorio entre 0 y un valor muy grande, usamos el módulo 100 para tener un rango entre 0 y 99
		print("la tubería 5 tiene grandes probabilidades de fallar...")
...
func _process(delta):
	...
	if failure_delay_timer > 0.0:
		...
		if failure_delay_timer <= 0.0:
			...
			print("¡La tubería 5 está fallando ahora!")
	...
	if Input.is_action_just_pressed("key_5"):  # Usamos la acción 'key_1'
		...
		if is_damaged:
			...
			print("¡La tubería 5 está fallando!")
			...
			print("¡La tubería está 5 en buen estado!")
